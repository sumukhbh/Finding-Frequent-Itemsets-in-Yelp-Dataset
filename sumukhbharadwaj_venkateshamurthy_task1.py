...
if case == 1:
      ...
      data = result.map(lambda a: a.split(","))
      new_data = data.map(lambda b: (b[1],[b[0]]))
      basket_data = new_data.reduceByKey(lambda a, b: a+b)

# Filter the Keys #
basket_rdd = basket_data.map(lambda a: (a[1]))
...
def a_priori(basket_data,support,total_baskets):
    ...
    while True:
        ...
        candidate_counts = defaultdict(lambda: 0) # To avoid KeyError #
        candidate_set = set()
        for basket in data:
            bas = set(basket)
...
converter = []
for a in Reduce_inter:
        if type(a) != str:
            converter.append(tuple(sorted(a)))
        else:
            converter.append(tuple([a]))

converter = sorted(converter, key=lambda a: (len(a), a))
...
Reducer_1 = Mapper_1.reduceByKey(lambda x, y: 1).collect()
#print(len(Reducer_1))
